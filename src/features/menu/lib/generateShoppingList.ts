import { Menu, IngredientInfo, IngredientCategory, ShoppingList, ShoppingListItem } from '../types';
 ...
 * 献立から全食材を抽出する
 */
export function extractIngredientsFromMenu(menu: Menu): Array<{ingredient: string, dish: string}> {
...
export function parseIngredient(ingredientText: string): IngredientInfo {
  // 分量を抽出（括弧内または最後の部分）
  const amountMatch = ingredientText.match(/[（(]([^）)]+)[）)]|(\d+[^\s）)]*)\s*$/);
  let amount = '';
  let name = ingredientText;
  
  if (amountMatch) {
    amount = amountMatch[1] || amountMatch[2] || '';
    name = ingredientText.replace(/[（(][^）)]+[）)]/, '').replace(/\d+[^\s）)]*\s*$/, '').trim();
  }
...
export function categorizeIngredient(ingredient: string): IngredientCategory {
  ...
  if (ingredient.match(/(しょうゆ|醤油|みりん|味噌|塩|油|酢|だし|ソース|スープ)/)) {
 ...
 * 同一食材の分量を統合する
 */
export function consolidateIngredients(ingredients: Array<{info: IngredientInfo, dish: string}>): ShoppingListItem[] {
...
export function generateShoppingList(
  ...
  // 2. 各食材をパースして詳細情報を取得
  const parsedIngredients = extractedIngredients.map(({ ingredient, dish }) => ({
  ...
  // 4. 既に持っている食材を除外
  const neededIngredients = consolidatedIngredients.filter(item => 
    !ownedIngredients.some(owned => 
...
export function formatShoppingListText(shoppingList: ShoppingList): string {
  const categoryGroups = shoppingList.items.reduce((groups, item) => {
